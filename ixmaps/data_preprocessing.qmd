...
`.data({url, type})` covers the common case: point a layer at a file, iXMaps fetches and binds it. When you need to **transform data before it reaches the map** — derive a column, aggregate rows, reshape a table — use [`Data.js`](https://gjrichter.github.io/docs/data.js/docs/index.html), iXMaps's own data-table library, which every layer's data pipeline is built on. This page covers the parts of the API used for map preprocessing; see the [full data.js documentation](https://gjrichter.github.io/docs/data.js/docs/index.html) for everything else (source-type reference, table transforms, joins).
...
- `.load(callback)` — starts loading; `callback(dataA)` receives a `Data.Table` **array**, indexed in `addSource` call order (`.realize(callback)` is an older name for the same method — still works)
...
:::{.callout-tip}
## Need to actually join the sources, not just load them together?
`Data.provider()` hands you back the loaded tables as a plain array — matching rows across them is still up to you. For a shared-key join (e.g. enriching one table's rows with columns from another), data.js also has `Data.merger()`. See [Combining Sources](https://gjrichter.github.io/docs/data.js/docs/combining_sources.html) in the data.js docs for both, with worked examples.
:::
...
This is the subset used most often for map preprocessing — `Data.Table` has more (`condense`, `sort`, `filter`, `groupColumns`, `append`, lookups, and the full `select`/`pivot` syntax). See [Transforming Tables](https://gjrichter.github.io/docs/data.js/docs/transforming_tables.html) and the [API Reference](https://gjrichter.github.io/docs/data.js/docs/api_reference.html) in the data.js docs for the complete method list.
