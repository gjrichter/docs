...
|---|---|---|---|
| 1 | Omitted `showdata: "true"` | Layer loads silently, nothing renders | Add `showdata: "true"` to every `.style()` |
...
The two-phase model: ixMaps loads and processes data (phase 1), then draws (phase 2). `showdata: "true"` is the gate that opens phase 2. Without it, data is fully loaded but never drawn.

**Checklist:**

1. **`showdata: "true"` missing** — most common issue
   ```javascript
   .style({ colorscheme: ["#0066cc"], showdata: "true" })
   ```

2. **`.binding()` missing or incorrect**
   ```javascript
   // ❌ Wrong:
...
3. **`.define()` missing** — the layer chain must end with `.define()`

4. **Data outside the visible map area** — zoom out to see if data appears elsewhere. Verify the map center/zoom matches the data's geographic location.

5. **Coordinates invalid** — latitude must be −90 to 90, longitude −180 to 180. Check for swapped lat/lon, null values, or coordinates stored as strings instead of numbers.

6. **Wrong visualization type for geometry**
...
6. ✅ Every layer has `showdata: "true"` in `.style()`
