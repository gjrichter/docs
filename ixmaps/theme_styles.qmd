...
| `colorscheme` | array / string | Color palette — see [Colors & Classification](colors_classification.qmd#color-schemes) |
...
| `sizepow` | number | Symbol dimension carrying the value: `1` = width, `2` = area (default), `3` = volume. Higher = compressed contrast — see [Sizing & Scaling](chart_scaling.qmd#sizepow-which-symbol-dimension-carries-the-value) |
...
| `rangecentervalue` | number | Diverging scale center — symmetrizes the range around this value regardless of class-count parity; an **even** color count is a visual-design choice for a clean split either side of center, not a requirement |
...
| `~~dopacityramp~~` | string | **Reserved, currently has no effect** — parsed but never read by the rendering code |
| `alphafield100` | string | Normalizes `alphafield` before computing opacity; the special value `"$density$"` divides it by polygon area — see [Colors & Classification](colors_classification.qmd#dynamic-opacity) |
...
| `alphafield100` | string | Normalizes `alphafield` before opacity is computed; `"$density$"` divides by polygon area — see [Colors & Classification](colors_classification.qmd#dynamic-opacity) |
...
| `colordef` | array / string | **Runtime only** (`changeThemeStyle`) — sets the fully-resolved color array directly, bypassing gradient/palette generation |
| `colorfield` | string | Binds color to a raw data field's value, independent of the display/label field — see [Colors & Classification](colors_classification.qmd#categorical-color-binding) |
| `colorscheme` | array / string | Color palette — single hex, gradient array, or named scheme — see [Colors & Classification](colors_classification.qmd) |
| `colorschemegeneration` | string | **Runtime only** (`changeThemeStyle`) — live-updates the gradient's mid-color (`"warm"` / `"cold"` / an explicit hex) |
| `colorstyle` | string | Style preset for a `spectrum` colorscheme, e.g. `"pastel"` — see [Colors & Classification](colors_classification.qmd#predefined-colorschemes) |
| `colorvalues` | array | Fixes the value → color order for `colorfield`, the same way `values` does for `CATEGORICAL` |
...
| `rangecentervalue` | number | Diverging scale center — symmetrizes regardless of class-count parity; even color count is a design choice, not a requirement |
| `rangescale` | number | Scale factor applied after range computation |
| `ranges` | string / array | Explicit class breaks: `n+1` values for `n` classes |
...
| `sizepow` | number | Symbol dimension carrying the value: `1` = width, `2` = area (default), `3` = volume — see [Sizing & Scaling](chart_scaling.qmd#sizepow-which-symbol-dimension-carries-the-value) |
| `sizevalueunits` | string | Units for the size value field |
