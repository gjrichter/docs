...
:::{.callout-important}
## `showdata: "true"` is always required
Every layer that should render anything must include `showdata: "true"` in its `.style()`. Without it, data loads and processes but the draw phase never starts — no error, just silence.
:::
...
|----------|------|-------------|
| `showdata` | string | **Required.** Must be the string `"true"` |
...
`clipupper` and `cliplower` are not valid style properties and are silently ignored.
:::
...
| `~~opacity~~` | number | **Deprecated** — use `fillopacity` |
...
| `showdata` | string | **Required** — must be `"true"` for any rendering to occur |
...
:::{.callout-note}
## Deprecated aliases
`fillcolor` → `colorscheme: ["#hex"]` · `strokecolor` → `linecolor` · `strokewidth` → `linewidth` · `opacity` → `fillopacity`
:::

:::{.callout-warning}
## Invalid properties (silently ignored)
`clipupper` and `cliplower` appear in some older documentation but are **not valid** style properties. Use `chartupper` / `chartlower` for scale-dependent chart visibility, or `layerupper` / `layerlower` for the full layer.
:::
