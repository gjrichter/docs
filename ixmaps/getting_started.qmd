---
title: "Quick Start"
description: "Build your first iXMaps map: one CDN script, an HTML skeleton, then chain layer, data, binding and type calls — or let an AI agent write it for you."
---
...
You don't have to write any of this by hand. Two AI-powered options turn a plain-language
description of your data into a working iXMaps map:

- **[Chat2Map.it](https://chat2map.it)** — explore your data, describe the visualization you
  want in plain language, and get a live, shareable map. No code, nothing to install.
- **[Agent-ready skills](https://github.com/gjrichter/ixmaps-skills)** — drop the iXMaps skills
  into Claude Code, Codex, or Cursor, and your AI agent writes runnable iXMaps code right in
  your editor.

Both know the iXMaps API and its common pitfalls. The hand-written walkthrough below is still
worth reading — it explains *what* the generated code does, so you can adjust the result with
confidence.
...
`data.js` is loaded automatically when `ixmaps.Map()` initializes — `Data.*` functions are available in any code that runs after that (layer callbacks, `.then()`, event handlers). Only if you need `Data` *before* creating the map do you add its own `<script>` tag — see [Data Preprocessing](data_preprocessing.qmd#loading-the-library).
:::
...
:::{.callout-tip}
## See it live
Open a live, editable version of this kind of bubble map in the
**[iXMaps code viewer](https://gjrichter.github.io/MapCodeViewer/index.html?url=maps/first_demo.html)** —
edit the code on the left and watch the map update on the right.
:::
...
- **[Map Setup](map_setup.qmd)** — map contexts, basemaps, projections
- **[Layers & Themes](layers_themes.qmd)** — the four steps that define every layer
- **[Visualization Types & Modifiers](map_types.qmd)** — the full `.type()` reference
- **[Colors & Classification](colors_classification.qmd)** — color schemes and class methods
- **[Sizing & Scaling](chart_scaling.qmd)** — symbol sizing across themes and zoom
- **[Tooltips & Interaction](tooltips_interaction.qmd)** — tooltips, events, runtime changes
