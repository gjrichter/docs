...
description: "iXMaps map contexts, basemap types and visualization types — choropleth, bubbles, charts — plus sizing and color rules and common pitfalls."
---
...
Declares whether the value is represented by the symbol's length (`1`), area (`2`, default) or volume (`3`); higher values compress contrast, values below `1` exaggerate it. See [Chart Scaling](chart_scaling.qmd#sizepow-which-symbol-dimension-carries-the-value) for the full definition and effect table:

```javascript
.style({ sizepow: 2 })  // area-proportional (default) — the cartographic standard
.style({ sizepow: 1 })  // width proportional to value — reads as exaggerated
```
...
Define the function on the `ixmaps` namespace (same convention as `userdraw` — see [User Charts](user_charts.qmd)). This is a real, shipping example — a `CATEGORICAL` speed-limit layer where the color depends on the text of each class label, not a fixed palette:
