...
:::{.callout-important}
## Projection maps require `.view([lat, lng], zoom)` array syntax
The object form `{ center: {lat, lng}, zoom }` does not work with projections. Always use the two-argument array form:
```javascript
.view([53.4, 16.9], 3.7)   // ✅ projection
.view({ center: {...}, zoom: 6 })  // ❌ Mercator only
```
:::
